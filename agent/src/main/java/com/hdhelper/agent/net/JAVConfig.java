package com.hdhelper.agent.net;
...
//TODO this should be in a commons-like jar
public class JAVConfig {
    ...
    public static JAVConfig decode(BufferedReader in) throws IOException {
        try {
            JAVConfig cfg = new JAVConfig();
            String line;
    ...
    public static JAVConfig decode(InputStream src) throws IOException {
        Reader reader = new InputStreamReader(src);
        BufferedReader buff_reader = new BufferedReader(reader);
        return decode(buff_reader);
    }

    public static JAVConfig decode(byte[] bytes) throws IOException {
