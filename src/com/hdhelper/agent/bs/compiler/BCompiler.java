...
public class BCompiler {
    ...
    public void inject(Class bs, ClassNode target) throws IOException {
        verify(bs, target);
        ClassNode from = load(bs);

        fixStaticOwners(from);
        appyCodec(from);
    ...
    public void inject(Class bs, Map<String,ClassNode> classes) throws IOException {
        ...
        ClassNode cn = classes.get(resolver.resolveName(name0));
            ...
            for (AbstractInsnNode ain : mn.instructions.toArray()) {
                if (ain instanceof MethodInsnNode) {
                    ...
                    if (resolver.hasPredicate(owner, name, desc, is_static)) {
                        ...
                        min.desc = GMethod.addPredicate(min.desc,ptype);
            ...
            for (AbstractInsnNode ain : mn.instructions.toArray()) {
                ...
                if(ain.getOpcode() == Opcodes.INVOKESTATIC) {
                    ...
                    if (resolver.hasCodec(owner, name, desc, true)) {
                        ...
                        codec = GPatch.toJavaNumber(codec);
